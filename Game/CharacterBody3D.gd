...
func shoot():
	var pos = Vector3(0.591, -0.027, -0.909)
	var tween = get_tree().create_tween()#.set_trans(Tween.TRANS_ELASTIC)
	tween.tween_property($Camera/MeshInstance3D, "position", pos+Vector3(0, 0, 0.5), 0.05)
	tween.tween_property($Camera/MeshInstance3D, "position", pos, 0.05)
	if $Camera/RayCast3D.is_colliding():
		var obj = $Camera/RayCast3D.get_collider() 
		if obj.has_meta("health"): obj.health -= 1
		else: print("Doesn't have health")#obj.call_deferred("queue_free")
